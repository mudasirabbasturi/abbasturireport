<nav class="navbar navbar-fixed-top">
    <div class="container-fluid">
        ...
        <div class="navbar-right pt-1">
            <a href="{{ route('pending') }}" class="btn l-amber btn-sm">
            ...
            <span style="position: relative;">
                ...
                <div class="dropdown-menu" style="position: absolute; transform: translate3d(0px, -2px, 0px); top: 100%; left: 0px; will-change: transform;">
                    <a class="dropdown-item" href="{{ route('pricingOnprogress') }}">
                        Pricing On Progress 
                        <span class="badge badge-primary">{{ $ProjectPricingOnProgressCount }}</span>
                    </a>
                    <a class="dropdown-item" href="{{ route('takeOfOnprogress') }}">
                        Takeoff On Progress 
                    ...
                    <a class="dropdown-item" href="{{ route('hold') }}">
                        Hold
                        <span class="badge badge-danger">{{ $ProjectHoldCount }}</span>
                    </a>
                    <a class="dropdown-item" href="{{ route('revision') }}">
                        Revision 
                        <span class="badge badge-danger">{{ $ProjectRevisionCount }}</span>
                    </a>
                </div>
            </span>
            <a href="{{ route('current.progress') }}" class="btn l-amber btn-sm" style="position: relative;">
                <small class="text-secondary">
                    <b>Current Progress 
                    </b>
                    <span class="badge badge-danger">{{ $ProjectCurrentCount }}</span>
                </small>
                <small class="currentprogress text-info border">
                   <b>Please Unlock If You Are Not A Part Of Any Projects.</b>
                </small>
            </a>
            <div id="navbar-menu">
...
document.addEventListener('DOMContentLoaded', function() {
      ...
      function updateNotificationCountHeader() {
      ...
      updateNotificationCountHeader();
      setInterval(updateNotificationCountHeader, 5000);
    });
